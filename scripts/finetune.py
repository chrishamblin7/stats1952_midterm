...
class Pruning_Finetuner:
	...
	def prune(self):
		...
		for _ in range(iterations):
			...
			for layer_index, filter_index in prune_targets:
				model = prune_conv_layer(model, layer_index, filter_index, use_cuda=args.use_cuda)
		...
		torch.save(model.state_dict(), "%s_prunned"%self.args.load_model.replace('models/','models/prunned/'))
...
def get_args():
	...
	parser.add_argument("--load_model", type = str, default = "models/custom_trained_enumandletters_seed5_0.955")
	parser.add_argument('--use-cuda', action='store_true', default=False, help='Use NVIDIA GPU acceleration')  
	parser.add_argument('--seed', type=int, default=2, metavar='S',
						help='random seed (default: 2)')
	parser.add_argument('--prune-rate', type=int, default=512, metavar='W',
						help='When pruning, how many filters should be deleted each iteration (default: 512)')
